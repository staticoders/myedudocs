...
const ManageCourses = () => {
    ...
    const columns = [
        ...
        { field: "cname", headerName: "Course Name", width: 200 },
        { field: "cinstructor", headerName: "Course Instructor", width: 200 },
        { field: "cdur", headerName: "Course Duration", width: 200 },
        { field: "clesson", headerName: "No Of Lesseons", width: 200 },
        { field: "cskill_level", headerName: "Skill Level Required", width: 200 },
        { field: "clanguage", headerName: "Language Of the Course", width: 200 },
        { field: "c_category", headerName: "Category", width: 200 },
        { field: "cprice", headerName: "Price", width: 200 },
        { field: "cdesc", headerName: "Description", width: 200 },
        { field: "approved_by_admin", headerName: "Course Approval", width: 200 },
        {
            field: "actions",
            headerName: "Actions",
            width: 300,
            renderCell: (params: any) => (
                <Stack direction="row" spacing={1}>
                    <a className='btn btn-primary' href={`/CourseDetails/${params.row._id}`} style={{ textDecoration: 'none', color: 'primary' }}>View</a>
                    <a className="btn btn-info" href={`/updateCourse/${params.row._id}`} style={{ textDecoration: 'none', color: 'primary' }}>Edit</a>
                    <Button onClick={
                        async () => {
                            try {
                                const response = await axios.delete(`http://localhost:8080/api/v1/course/deleteCourse/${params.row._id}`);
                                // console.log(response.data); // Handle success response
                                message.success("Course Deleted Successfully", 4)
                                ...
                                message.error("Error Deleting Course", 4)
                            }
                        }
                    ...
                    {"aname" in loginUser && (
                        <Button
                            onClick={
                                async () => {
                                    try {
                                        const response = await axios.put(`http://localhost:8080/api/v1/course/UpdateCourseApprovalStatus/${params.row._id}`);
                                        // console.log(response.data); // Handle success response
                                        message.success("Course Approved Successfully", 4)
                                        // window.location.reload()
                                        setTimeout(() => {
                                            window.location.reload(); // Reloads the current page
                                        }, 4000);
                                        ...
                                        console.error(error); // Handle error response
                                        message.error("Error Course Approved", 4)
                                    }
                                }
                            }
                            variant="contained"
                            ...
                            <div className="row">
                                <div className="col-md-12">
                                    <div style={{ height: 400, width: "100%" }}>
                                        <DataGrid
                                            rows={Array.isArray(Courses) ? Courses.map((course, index) => ({ ...course, id: index + 1 })) : []}
                                            columns={columns}
                                            initialState={{
                                                pagination: {
